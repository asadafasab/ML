...
class Decoder(nn.Module):
    def __init__(
        ...
        super().__init__()
        self.device = device
        self.word_embeding = nn.Embedding(trg_vocab_size, embed_size)
        self.position_embeding = nn.Embedding(max_length, embed_size)
    ...
    def forward(self, x, enc_out, src_mask, trg_mask):
        N, seq_lenght = x.shape
        positions = torch.arange(0, seq_lenght).expand(N, seq_lenght).to(self.device)
        x = self.dropout((self.word_embeding(x) + self.position_embeding(positions)))
...
class Transformer(nn.Module):
    ...
    def forward(self, src, trg):
        src_mask = self.make_scr_mask(src)
